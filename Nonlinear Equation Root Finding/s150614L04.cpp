...
void finalOutput(int problemID, double parameters[], double ans, 
	...
	case PROBLEM2:
		...
		break;
	case PROBLEM3:
		ans1 = parameters[0] * ans + parameters[1];
		printf("ans: x= %.18e y= %.18e |f(x)|= %.10e iteration= %2d\n", ans, ans1, absfval, iteration);
		break;
	default:
		printf("Problem id = %d is not defined\n", problemID);
		exit(-1);
...
void Secant (
	...
	for (iter = 0; iter <= maxIter; iter++) {
		...
		// 다음 두 줄이 Secant method core 입니다
		x2 = x1 - (_F(x1, p[0], p[1], p[2]) * ((x1 - x0) / (_F(x1, p[0], p[1], p[2]) - _F(x0, p[0], p[1], p[2]))));
		x0 = x1;
		x1 = x2;
	}
	// 최종 출력을 저장합니다
...
void Bisection(
	...
	for (iter = 0; iter <= maxIter; iter++) {
		...
		// 다음 두 줄이 Secant method core 입니다
		x0 = x1;
		if ((_F(x1, p[0], p[1], p[2])*_F(L, p[0], p[1], p[2])) < 0) R = x1;
		else L = x1;
	}
	// 최종 출력을 저장합니다
	*iteration = iter;	// 총 반복 회수
