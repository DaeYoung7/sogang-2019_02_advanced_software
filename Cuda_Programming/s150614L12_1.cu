...
__global__ void Sum_n_elements_Kernel_No_shared(IN int *d_ArrayElements, OUT int *d_SumOfArrayElements, int N, int Nf) {
	...
	//TODO
	for (int i =  - Nf; i <=  + Nf; i++)
		if (i+ id >= 0 && i+ id < N) sum += d_ArrayElements[i+ id];
...
__global__ void Sum_n_elements_Kernel_shared(IN int *d_ArrayElements, OUT int *d_SumOfArrayElements, int N, int Nf) {
	...
	shared_buffer[thread_id] = 0;
	shared_buffer[thread_id + Nf] = 0;
	shared_buffer[thread_id + 2 * Nf] = 0;
	if (id >= Nf)
		shared_buffer[thread_id] = d_ArrayElements[id-Nf];
	if ((id + Nf) < N)
		shared_buffer[thread_id + 2 * Nf] = d_ArrayElements[id + Nf];
...
int main()
{
	...
	printf("\n*** CPU[10] : %d GPU(No S)[10] : %d GPU(shared)[10] : %d\n", h_SumOfArrayElements_CPU[10], h_SumOfArrayElements_GPU_No_Shared[10], h_SumOfArrayElements_GPU_Shared[10]);
