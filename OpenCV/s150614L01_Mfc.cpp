...
void SWL01::drawImage(CDC *pDC, int dcLTx, int dcLTy, int which) {
	if (which == 1) {
		...
		dcLTx, dcLTy,// 화면의좌상귀좌표 
		m_MatR.cols, m_MatR.rows,// 화면의폭과높이 
		0, 0,// Mat 배열의좌상귀 
		m_MatR.cols, m_MatR.rows,// Mat 배열의폭과높이 
		m_MatR.data,// image data to display 
		mg_lpBMIHR,// BITMAPINFO 시작주소 
		DIB_RGB_COLORS,// RGB 또는색상테이블인덱스 
			SRCCOPY// 래스터연산방법 
		); }
}
...
void SWL01::DIBtoMat(int which) {
	...
	if (depth == 16) {
		for (int p = 0; p < h * w * (*pMat).channels() * 2; p++)
			*pMatData++ = *pDibData++;
	}
